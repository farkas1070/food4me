...
const AddFoodModal = ({ isVisible, onClose, options }) => {
  ...
  const onAddPress = async (selectedOption) => {
    try {
      const docRef = doc(db, "Recipes", selectedOption.docid);
      
  
      const nutrientQuery = query(
        collection(db, "Recipe_Nutrition"),
        where("Recipe_ID", "==", docRef),
      ...
      if (!querySnapshot.empty) {
        ...
        const calorieAmount = matchingDocument.data().amount;
        console.log('Amount for selected food:', calorieAmount);
      } else {
